...
export const metadata: Metadata = {
  ...
  description: 'Future TWKH field guides, books, editorial prints and habitat-inspired objects available independently of The Habitat Portrait.',
};
  ...
  return (
    <main>
      ...
      <PageHero
        ...
        intro="The TWKH Shop will offer independently purchasable field guides, editorial prints and considered habitat goods. No Habitat Portrait will be required."
        dark
      />

      <section className="shop-boundary-banner">
        <ShoppingBag size={29} />
        <div><p className="kicker">Available to everyone</p><h2>This is the independent Shop.</h2><p>Everything shown here can be purchased independently of The Habitat Portrait. Property-specific photographs and custom habitat portraits remain Habitat Portrait add-ons under Services.</p></div>
      ...
      <section className="shop-plans">
        ...
        <div className="shop-grid">
          {shopPlans.map(({ icon: Icon, tag, title, text }, index) => <article key={title}><span className="product-number">0{index + 1}</span><Icon size={26} /><p className="product-tag">{tag}</p><h2>{title}</h2><p>{text}</p><span className="in-development">No Habitat Portrait required · In development</span></article>)}
        </div>
      </section>
